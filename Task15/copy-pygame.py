...
monster_enemy = pygame.image.load("monster.jpg")
killer = pygame.image.load("killer.jpg")
...
monster_enemy_height = enemy.get_height()
monster_enemy_width = enemy.get_width()
killer_height = killer.get_height()
killer_width = killer.get_width()
...
print("This is the height of the monster_enemy: " + str(image_height))
print("This is the width of the monster_enemy: " + str(image_width))
print("This is the width of the prize:  "  + str(image_width))
print("This is the height of the prize: "  + str(image_width))
...
killerXPosition = screen_width
killerYPosition = random.randint(0, screen_height - killer_height)
monster_enemyXPosition = screen_width
monster_enemyYPosition = random.randint(0, screen_height - monster_enemy_height)
...
while True : # This is a looping structure that will loop the indented code until you tell it to stop(in the event where you exit the program by quitting). In Python the int 1 has the boolean value of 'true'. In fact numbers greater than 0 also do. 0 on the other hand has a boolean value of false. You can test this out with the bool(...) function to see what boolean value types have. You will learn more about while loop structers later. 
    ...
    screen.blit(killer, (killerXPosition, killerYPosition))
    screen.blit(monster_enemy, (monster_enemyXPosition, monster_enemyYPosition))
    ...
    killerBox = pygame.Rect(killer.get_rect())
    killerBox.top = killerYPosition
    killerBox.left = killerXPosition
    
    monster_enemyBox = pygame.Rect(monster_enemy.get_rect())
    monster_enemyBox.top = monster_enemyYPosition
    monster_enemyBox.left = monster_enemyXPosition
    ...
    if playerBox.colliderect(killerBox):
        print("You lose")
        pygame.quit()
        exit(0)

    
    if playerBox.colliderect(monster_enemyBox):
        print("You lose")
        pygame.quit()
        exit(0)
    ...
    enemyXPosition -= 0.15
    killerXPosition -= 0.60
    monster_enemyXPosition -= 0.30
    prizeXPosition -= 0.25
